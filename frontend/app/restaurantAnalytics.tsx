      ...
      try {
        ...
        if (metricsData.menu_item_metrics.length > 0) {
          const totalRating = metricsData.menu_item_metrics.reduce(
            (acc, item) => acc + item.overall_rating, 0
          );
          const avgRating = parseFloat(
            (totalRating / metricsData.menu_item_metrics.length).toFixed(1)
          );
          ...
          const totalReturnRate = metricsData.menu_item_metrics.reduce(
            (acc, item) => acc + item.return_rate, 0
          );
  ...
  return (
    <ThemedView style={[styles.container, { paddingTop: insets.top }]}>
      ...
      >
        ...
        {metrics.menu_item_metrics.length > 0 && metrics.menu_item_metrics[0].dietary_restrictions.length > 0 && (
          <View style={styles.tagsContainer}>
