...
// GetUserFollowing retrieves a paginated list of users that the specified user follows
func (s *Service) GetUserFollowing(userId string, page, limit int) ([]UserResponse, error) {
	...
	followeeIds := make([]primitive.ObjectID, len(connections))
	for i, conn := range connections {
		followeeIds[i] = conn.FolloweeId
	}
	...
	usersCursor, err := s.users.Find(ctx,
		bson.M{"_id": bson.M{"$in": followeeIds}},
	)
	if err != nil {
		return nil, err
	}
...
// GetFollowingFeed retrieves recent reviews from followed users
func (s *Service) GetFollowingFeed(userId string, page, limit int) ([]review.ReviewDocument, error) {
	ctx := context.Background()
	userObjID, err := primitive.ObjectIDFromHex(userId)
	if err != nil {
		badReq := xerr.BadRequest(err)
	...
	reviewsCursor, err := s.reviews.Find(ctx,
		bson.M{"reviewer.id": bson.M{"$in": followeeIds}},
		options.Find().
